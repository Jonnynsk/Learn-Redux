...
Все reducers приложения собирает Store, формируя таким образом общее изначальное состояние для приложения.
    ...
    Хорошей практикой является деление reducers на более компактные. 
    Каждый reducer отвечает за свою часть состояния и его изменения. 
    Далее rootReducer склеивает все reducers и возвращает новое состояние и передает его store, а store уже обновляет приложение. 
